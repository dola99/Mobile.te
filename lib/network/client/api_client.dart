...
import 'package:mobility/network/services_urls.dart';
...
class ApiClient {
  ...
  static Future<Response> getRequest(
    ...
    final Uri url = Uri(
      scheme: ServicesURLs.developmentEnvironmentScheme,  // Todeo add Url
  ...
  static Future<http.Response> postRequest(String endPoint, dynamic requestBody,
    ...
    if (!isMultipart) {
      ...
      final Map<String, dynamic> p = jsonDecode(requestBody.toString())as Map<String,dynamic>;
  ...
  static Future<http.Response> putRequest(String endPoint, dynamic requestBody,
   ...
   log('$url "\n"  ${ApiClient().headers().toString()}');
    ...
    if (!isMultipart) {
      log("*NotMultipart*");
      response =
          await http.put(Uri.parse(url), headers: ApiClient().headers(), body: requestBody);
    } else {
      final uri = Uri.parse(url);
      final Map<String, dynamic> p = jsonDecode(requestBody.toString())as Map<String,dynamic>;
     final Map<String, String> convertedMap = {};
